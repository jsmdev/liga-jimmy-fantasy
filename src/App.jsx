...
export default function App(){
  ...
  useEffect(()=>{ (async ()=>{
    setLoading(true)
    const { data: parts } = await supabase.from('participants').select('id,name,team_name,photo_url').order('name')
    const { data: pens }  = await supabase.from('penalties').select('id,participant_id,amount,reason,date').order('date', { ascending:false })
    setParticipants(parts||[]); setPenalties(pens||[]); setLoading(false)
  })() }, [])
            ...
            <section>
              <Card className="overflow-hidden">
                <CardHeader>
                  ...
                  <CardDescription>Consulta detallada de sanciones y bonificaciones.</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="rounded-xl overflow-hidden border border-slate-200">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filtered.length===0 ? (
                          <tr><td colSpan={6} className="text-center py-10 text-slate-500">No hay penalizaciones aún.</td></tr>
                        ) : filtered.map(pen=>{
