          ...
          <div className="flex">
            {photos.map((p, i)=>(
              <div key={i} className="min-w-0 flex-[0_0_80%] sm:flex-[0_0_33%] md:flex-[0_0_25%] lg:flex-[0_0_20%] p-2">
                <div
                  className="aspect-square rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 cursor-zoom-in"
                  onClick={() => openLightbox(slides, { index: i })}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      event.preventDefault()
                      openLightbox(slides, { index: i })
                    }
                  }}
                >
